...
public class Player extends offset.sim.Player {
    ...
    int minimax_run(int current_depth, int max_depth, boolean my_turn, movePair next_best_move, boolean verbose)
    {
        ...
        if((current_depth == max_depth) /*|| no moves left*/ )
        {
            ...
            return advGridGraph.getNumberOfEdges();
        }
        ...
        if (my_turn)
        {
            best_value = 1000000; //Because I have to minimize
            ...
            while(it.hasNext())
            {
                ...
                while(it_n.hasNext())
                {
                    ...
                    value = minimax_run(current_depth+1, max_depth, !my_turn, child_best_move, verbose);
                    if(verbose)
                    {
                    ...
                    if (value <= best_value)//Minimize, Maximize best value
                    {
        ...
        {
            best_value = -1;//Because Adv has to maximize
            ...
            while(it.hasNext())
            {
                ...
                while(it_n.hasNext())
                {
                    ...
                    value = minimax_run(current_depth+1, max_depth, !my_turn, child_best_move, verbose);
                    if (value >= best_value)//Maximize, Minimize best value
                    {
                        best_movepr.src = movepr.src;
                        best_movepr.target = movepr.target;
                        best_movepr.move = true;
